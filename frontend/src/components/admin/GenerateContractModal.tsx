...
import { X, AlertCircle, FileSignature, Check } from 'lucide-react'
import { AxiosError } from 'axios'
import { Button } from '@/components/ui/Button'
import { useGenerateContract } from '@/hooks/admin/useAdminContracts'
...
export function GenerateContractModal({ onClose }: Props) {
  ...
  function handleGenerate() {
    ...
    generate.mutate(selectedId, { onSuccess: () => onClose() })
  }
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto p-4 py-8">
      ...
      <div className="relative w-full max-w-2xl rounded-2xl bg-cream shadow-2xl">
        <div className="flex items-center justify-between border-b border-line px-6 py-4">
          <div>
            <h2 className="font-display text-xl font-bold text-ink">Gjenero kontratë</h2>
            <p className="mt-0.5 text-xs text-muted">Zgjidh një rental për kontratën</p>
          </div>
          ...
          <div className="max-h-96 overflow-y-auto rounded-xl border border-line bg-white">
            ...
            {data && data.data.length === 0 && (
              <div className="p-8 text-center">
                ...
                <p className="mt-3 text-sm text-muted">Asnjë rental. Krijo rental fillimisht.</p>
              </div>
            )}
        ...
        <div className="flex justify-end gap-3 border-t border-line px-6 py-4">
          ...
          >
            ...
            {generate.isPending ? 'Duke gjeneruar…' : 'Gjenero kontratë'}
          </Button>
        </div>
      </div>
    </div>
